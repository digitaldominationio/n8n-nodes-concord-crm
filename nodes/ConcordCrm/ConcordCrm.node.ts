...
export class ConcordCrm implements INodeType {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        const getFields = (paramName: string): IDataObject => {
          const fields = this.getNodeParameter(paramName, i, {}) as IDataObject;
        ...
        // ────────────────────────────────────────────────────────
          ...
          const trashedResource = this.getNodeParameter('trashedResource', i) as string;

          if (operation === 'list') {
            const page = this.getNodeParameter('page', i, 1) as number;
            const perPage = this.getNodeParameter('perPage', i, 15) as number;
        ...
        // ────────────────────────────────────────────────────────
          ...
          const commentableResource = this.getNodeParameter('commentableResource', i) as string;
          const commentableId = this.getNodeParameter('commentableId', i) as string;

          if (operation === 'list') {
            responseData = await concordRequest.call(this, 'GET', `/${commentableResource}/${commentableId}/comments`);
          } else if (operation === 'create') {
            const body = this.getNodeParameter('commentBody', i) as string;
        ...
        // ────────────────────────────────────────────────────────
          ...
          if (operation === 'list') {
            ...
            const qs: IDataObject = { page, per_page: perPage, ...additionalFilters };
            ...
            const recordId = this.getNodeParameter('recordId', i) as string;
            const assocResource = this.getNodeParameter('associationResource', i) as string;
            const rawIds = this.getNodeParameter('associationIds', i) as string;
